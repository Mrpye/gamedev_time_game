...
public class InputReplay : MonoBehaviour {
	...
	public delegate float GetAxisRawType(string name);
	...
	public GetAxisRawType GetAxisRaw;
	...
	void FixedUpdate() {
		if (UpdateCycle == UpdateFunction.FixedUpdate || UpdateCycle == UpdateFunction.Both)
			if (mode == Mode.Record) {
				...
				work(Time.fixedTime - (startTime - 1));
			}
	}
	...
	public void Stop() {
		...
		switch (mode) { // close streams
			case Mode.Record:
				if (inputRecordStream!=null) { inputRecordStream.Close(); }
				
				break;
			case Mode.PlayBack:
				if (inputPlaybackStream != null) {
	...
	private void SetInputFake() // redirect public methods and properties to our replay system
	{
		...
		GetAxisRaw = delegate (string name) { return currentSequence.vA.ElementAt(AxisList.FindIndex(str => str == name)); };
	...
	private void Record(float time) {
		...
		foreach (string virtualAxis in AxisList)
			currentSequence.vA.Add(Input.GetAxisRaw(virtualAxis));
